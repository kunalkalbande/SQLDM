...
import com.idera.sqldm_10_3.data.topten.WaitWidgetInstance;
...
public class InstancesByWaitsComposer extends DashboardTopXWidgetsComposer {

	private static final long serialVersionUID = 1L;
	private ListModelList<WaitWidgetInstance> listModel;
	private Logger logger = Logger.getLogger(InstancesByWaitsComposer.class);

	@Override
	public TypeReference<?> getModelType() {
		return new TypeReference<List<WaitWidgetInstance>>() {
		};
	}
	...
	public String getUtilization(IWidgetInstance instance) {
		String utilization = "" + ((WaitWidgetInstance)instance).getWaitTime();
		return utilization;
	}
	...
	public String getEventName() {
		return String.format("%s:%s", config.getId(),
				WaitWidgetInstance.class.getName());
	}
	...
	public void setListModel(Object obj) {
		listModel = new ListModelList<WaitWidgetInstance>(
				(Collection<? extends WaitWidgetInstance>) obj);

		Collections.sort(listModel, new Comparator<WaitWidgetInstance>() {
			@Override
			public int compare(WaitWidgetInstance o1, WaitWidgetInstance o2) {
				Double v1 = o1.getWaitTime();
				Double v2 = o2.getWaitTime();
