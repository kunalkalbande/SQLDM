...
import com.idera.sqldm.data.SQLInstanceCounts;
import com.idera.sqldm.data.UserSettings;
import com.idera.sqldm.data.alerts.Alert;
import com.idera.sqldm.data.alerts.AlertMetaData;
import com.idera.sqldm.data.alerts.AlertMetrics;
...
import com.idera.sqldm.data.databases.Database;
import com.idera.sqldm.data.databases.DatabaseCapacityUsageDetails;
import com.idera.sqldm.data.databases.DatabaseDetails;
...
import com.idera.sqldm.ui.dashboard.GearViewBean;
...
public class SQLDMRestClient {
	...
	public <T> T getProductRestResponseCore(String productInstanceName, String methodCallUrl,
		...
		try {
			...
			productInstance = (Product) com.idera.sqldm.server.web.session.SessionUtil.getSessionVariable("currentSqldmProduct");
			if (productInstanceName != null) {
				if (productInstance == null
					...
					com.idera.sqldm.server.web.session.SessionUtil.setSessionVariable("currentSqldmProduct",
							productInstance);
				}
			...
			try {
				...
				curr = (ICSTokenCredentials)SessionUtil.getCurrentUserCredentials();
				if (curr != null) {
	...
	public <T> T deleteProductRestResponseCore(String productInstanceName, String methodCallUrl, Object requestObject,
		...
		try {
			...
			Product productInstance;
			if (productInstanceName == null) {
				productInstance = (Product) com.idera.sqldm.server.web.session.SessionUtil
	...
	public String getVersion(String productInstanceName) throws RestException {
		try {
			List<Product> products = CoreRestClient.getInstance().getProductInstances(SQLdmConstants.PRODUCT_NAME, null, productInstanceName);
			if(null!=products && products.size()!=0) {
				return products.get(0).getVersion();
		}
		}catch(Exception e) {
			log.error("Error in getting the version of product with product Instance Name:"+productInstanceName+" and error: "+e.getMessage());
		}
		return null;
	}
...
//--------------------//
	...
	//Method for PA
	public List<PrescreptiveAnalysisRecord> getPrescreptiveAnalysisForInstance(String instanceId) throws RestException {
		try {
			String url = RestMethods.GET_PRESCREPTIVE_ANALYSIS.getMethodName();
			url = url.replace("{INSTANCEID}", new Integer(instanceId).toString());
			...
			return getProductRestResponseCore(
				productInstanceName, 
				url, 
				new TypeReference<ArrayList<PrescreptiveAnalysisRecord>>() {}
			);
		} catch (RestException x) {
			throw this.getRestException(I18NStrings.EXCEPTION_OCCURRED_GETTING_ALL_CORE_SERVICES_INSTANCES, x);
		}
	...
	//Method for PA details
	public List<Recommendation> getPrescreptiveAnalysisDetailsForInstance(String instanceId, String id) throws RestException {
		try {
			String url = RestMethods.GET_PRESCREPTIVE_ANALYSIS_DETAILS.getMethodName();
			url = url.replace("{INSTANCEID}", new Integer(instanceId).toString());
			url = url.replace("{ANALYSISID}", new Integer(id).toString());

			String productInstanceName = Utility.getUrlParameter(Executions.getCurrent().getParameterMap(), "instance");
			return getProductRestResponseCore(
				productInstanceName, 
				url, 
				new TypeReference<ArrayList<Recommendation>>() {}
			);
		} catch (RestException x) {
			throw this.getRestException(I18NStrings.EXCEPTION_OCCURRED_GETTING_ALL_CORE_SERVICES_INSTANCES, x);
		}
