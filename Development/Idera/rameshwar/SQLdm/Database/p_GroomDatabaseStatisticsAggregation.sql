...
if (object_id('Grooming.p_GroomDatabaseStatisticsAggregation') is not null)
begin
drop procedure Grooming.p_GroomDatabaseStatisticsAggregation
end
go
create procedure [Grooming].[p_GroomDatabaseStatisticsAggregation]
(
...
Declare @KeysToDelete Table (AggregatedDatabaseStatisticsID bigint not null primary key)

select 
	@RowsAffected = 0, 
	@RC = 0, 
	@BlockName = 'DatabaseStatisticsAggregation'
	
if (@SQLServerID is not null)
begin
	Set @cmd = N'Declare @KeysToDelete Table (AggregatedDatabaseStatisticsID bigint not null primary key)
			
			Insert Into @KeysToDelete
			Select Top(@RecordsToDelete) AggregatedDatabaseStatisticsID
			From dbo.DatabaseStatisticsAggregation ds with(nolock)
			Inner Join SQLServerDatabaseNames dn with(nolock) on ds.DatabaseID = dn.DatabaseID
			Where SQLServerID = @SQLServerIDIn' +
				Case When @Deleted = 0 Then N'
				And ds.MaxUTCCollectionDateTime <= @CutoffDateTimeIn'
				Else N''
				End + N';
		
			Delete DS
			From dbo.DatabaseStatisticsAggregation DS
			Inner Join @KeysToDelete K
				On K.AggregatedDatabaseStatisticsID = DS.AggregatedDatabaseStatisticsID;';
...
begin
	...
	begin try
		...
		begin
			...
			Insert Into @KeysToDelete
			Select Top(@RecordsToDelete) AggregatedDatabaseStatisticsID
			From dbo.DatabaseStatisticsAggregation with(nolock)
