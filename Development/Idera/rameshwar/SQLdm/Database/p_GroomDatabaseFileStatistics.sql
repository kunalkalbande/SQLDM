if (object_id('Grooming.p_GroomDatabaseFileStatistics') is not null)
begin
drop procedure Grooming.p_GroomDatabaseFileStatistics
end
go
create procedure [Grooming].[p_GroomDatabaseFileStatistics]
(
...
select 
	@RowsAffected = 0, 
	@RC = 0, 
	@BlockName = 'DatabaseFileStatistics'
...
begin
	...
	Set @cmd = N'Declare @KeysToDelete Table (UTCCollectionDateTime datetime not null,
			...
			From dbo.DatabaseFileStatistics dfa with(nolock)
			...
			Delete DFA
			From dbo.DatabaseFileStatistics DFA
	...
	if @Deleted = 0
	begin
		set @cmd = @cmd + ' and DFA.UTCCollectionDateTime <= @CutoffDateTimeIn'
	end
	else
	begin
...
begin
	...
	begin try
		...
		begin
			...
			From dbo.DatabaseFileStatistics with(nolock)
