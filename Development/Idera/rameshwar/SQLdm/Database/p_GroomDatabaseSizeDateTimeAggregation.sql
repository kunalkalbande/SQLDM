...
if (object_id('Grooming.p_GroomDatabaseSizeDateTimeAggregation') is not null)
begin
drop procedure Grooming.p_GroomDatabaseSizeDateTimeAggregation
end
go
CREATE procedure [Grooming].[p_GroomDatabaseSizeDateTimeAggregation]
(
...
Declare @KeysToDelete Table (AggregatedDatabaseID bigint not null primary key)

select 
	@RowsAffected = 0, 
	@RC = 0, 
	@BlockName = 'DatabaseSizeDateTimeAggregation'
	
if (@SQLServerID is not null)
begin
	Set @cmd = N'Declare @KeysToDelete Table (AggregatedDatabaseID bigint not null primary key)
			
			Insert Into @KeysToDelete
			Select Top(@RecordsToDelete) AggregatedDatabaseID
			From dbo.DatabaseSizeDateTimeAggregation ds with(nolock)
			Inner Join dbo.SQLServerDatabaseNames dn with(nolock) on ds.DatabaseID = dn.DatabaseID
			...
			Delete DS
			From dbo.DatabaseSizeDateTimeAggregation DS
			Inner Join @KeysToDelete K
				On K.AggregatedDatabaseID = DS.AggregatedDatabaseID;';
...
begin
	...
	begin try
		...
		begin
			...
			Insert Into @KeysToDelete
			Select Top(@RecordsToDelete)  AggregatedDatabaseID
			From dbo.DatabaseSizeDateTimeAggregation with(nolock)
