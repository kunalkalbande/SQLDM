...
IF NOT EXISTS (SELECT * FROM tempdb..sysobjects WHERE id =OBJECT_ID('tempdb..##dmfragmentationworktable'))
begin
	create table ##dmfragmentationworktable
	...
	delete from ##dmfragmentationworktable where servicename = host_name() and runtime != convert(datetime, '{2}', 121)

if not exists(select top 1 * from ##dmfragmentationworktable where servicename = host_name() and runtime = convert(datetime, '{2}', 121))
begin
		insert into ##dmfragmentationworktable(servicename,runtime) values (host_name(),convert(datetime, '{2}', 121))
	...
	begin
		...
		begin 
			set @command = 
				'insert into ##dmfragmentationworktable(dbid,objectid,schemaname,objectname,servicename,runtime)
				select 
				@DBID,
				i.object_id,
				s.name,
