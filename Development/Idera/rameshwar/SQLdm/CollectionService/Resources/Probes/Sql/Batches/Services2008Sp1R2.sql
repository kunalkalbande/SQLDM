...
declare 
	...
	@directwmi int,
	@defaultInstanceAgentName  varchar(50)

select @defaultInstanceAgentName = 'SQLSERVERAGENT'	
...
begin
	...
	if @slashpos <> 0 
		...
		begin 
			...
			select @agentservicename = @defaultInstanceAgentName 
	...
	SELECT @service_status = sys.dm_server_services.status FROM sys.dm_server_services where servicename = @sqlservicename 
	
	if @service_status is null and @slashpos <> 0
	begin
		select @sqlservicename = 'SQL Server (' + substring(@servername, @slashpos + 1, 30) + ')' 
		SELECT sys.dm_server_services.status_desc FROM sys.dm_server_services where servicename = @sqlservicename 
	end
	...
	begin 
		...
		begin 
			...
			declare @oleAgentServiceName varchar(MAX)
			
			if (@agentServicename = 'SQL Server Agent (MSSQLSERVER)')
				set @oleAgentServiceName = @defaultInstanceAgentName -- Set the name of the default instance agent name.
			else
				set @oleAgentServiceName = @agentservicename
