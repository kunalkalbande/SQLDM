-- SQLdm 8.6  Hashvalue: o/ACzC5B0n9AwnZqxp3BCL3At9U= 
--------------------------------------------------------------------------------
...
select name,
	database_id as dbid, 
    0 as mode, 
    case when name in ('master','model','msdb','tempdb') then cast(1 as bit) 
    else is_distributor end,
    ...
    compatibility_level as cmptlevel,
	NULL as roleAG
 from sys.databases db (nolock)
 where lower(name) <> 'mssqlsystemresource'
 order by user_access, state
for read only
set nocount on 
open read_db_status 
	...
	begin 
		...
		begin
			...
			from 
				...
				left join
				#disk_drives
				on lower(substring(filename,1,len(drive_letter))) ' + case when @cmptlevel > 70 then ' collate database_default ' else ' ' end + ' = lower(drive_letter) ' + case when @cmptlevel > 70 then ' collate database_default ' else ' ' end + '
