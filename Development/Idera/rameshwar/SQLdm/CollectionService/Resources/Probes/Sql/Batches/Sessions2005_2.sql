...
--------------------------------------------------------------------------------
--START (RRG): This script will not work on master but will on User Databases
--     make sure that a User Database connection is select when using this
--     script on Azure SQL DB

--use {3}  --USE not supported in Azure SQL Database, must use User Database connection
...
--START (RRG): dm_tran_active_snapshot_database_transactions' is avail
...
--END (RRG): dm_tran_active_snapshot_database_transactions' is avail

--START (RRG): 'sysprocesses' is avail
...
TempdbQueries as(
	select {2}
		...
		sess.program_name,
		req.command,
		databaseName = db_name(req.database_id),--databaseName = coalesce(db_name(req.database_id)),--,db_name(ot.database_id)),
		...
		null as net_transport,--ot.net_transport,
		null as client_net_address,--rtrim(ot.client_net_address) as client_net_address,
		wait_time,
		req.request_id,
		last_wait_type,
		wait_type,
