...
begin 
	create table #filestats (Fileid int, FSFileGroup int, TotalExtents decimal(38,0), UsedExtents decimal(38,0), FSName sysname, FSFileName nchar(520))
end
else
begin
...
+ ' 
 ...
 BEGIN
   INSERT #filestats
   SELECT f.fileid AS Fileid, f.groupid AS FileGroup, f.size/8 AS TotalExtents, fileproperty(f.name,''SpaceUsed'')/8 AS UsedExtents, f.name AS Name, master.physical_name AS FileName
   FROM sys.sysfiles f
   JOIN  sys.master_files   master on f.fileid=master.file_id and master.database_id=DB_ID('+''''+@dbname+''''+')
   JOIN sys.database_files db_f ON f.fileid = db_f.file_id
   JOIN sys.data_spaces ds ON ds.data_space_id = db_f.data_space_id
    
 END
 declare @TempSysfiles table(drivename nvarchar(256), length int, filename nvarchar(512), fileid smallint, name sysname, status int, growth decimal(38,0), maxsize decimal(38,0), size decimal(38,0), groupid smallint)
 declare @Sysfiles table(drivename nvarchar(256), filename nvarchar(512), fileid smallint, name sysname, status int, growth decimal(38,0), maxsize decimal(38,0), size decimal(38,0), groupid smallint)
 insert into @TempSysfiles select drivename = drive_letter, length = len(drive_letter),masterstat.physical_name,sys.fileid,sys.name,status,sys.growth,maxsize,sys.size,groupid from sysfiles sys JOIN sys.master_files masterstat on sys.fileid = masterstat.file_id and masterstat.database_id=DB_ID('+''''+@dbname+''''+')  left join #disk_drives'
-- Protect againt using "collate" on SQL 7 compatible dbs
if (@cmptlevel > 70)
	select @command = @command + ' on lower(substring(masterstat.physical_name,1,len(drive_letter))) collate database_default = lower(drive_letter) collate database_default '
else
	select @command = @command + ' on lower(substring(masterstat.physical_name,1,len(drive_letter))) = lower(drive_letter) '	
...
+  quotename(@dbname,'''') + ', 
	name, 
 case when f.status&1000000=0 then 1 else 0 end, case when f.status&1000000=0 then groupname else ''n/a'' end, growth, TotalSize = case when f.status&1000000=0 then (convert(dec(38,5),TotalExtents) * 64) else 
...
select @command = @command + 'end, 
 UsedSize = case when f.status&1000000=0 then (convert(dec(38,5),UsedExtents) * convert(dec(38,5),64)) else (select max(convert(dec(38,5),logsize) * (convert(dec(38,5),logspaceused))/100 * 1024) from #sqlperf where rtrim(databasename) = rtrim(' + quotename(@dbname,'''') + ')) end, 
	configuredmaxsize = maxsize, expansionsize = ( select convert(dec(38,5),isnull( case when maxsize > 0 and (convert(dec(38,5),maxsize)*8)-(convert(dec(38,5),size)*8) <= (convert(dec(38,5),d.unused_size)/1024)  
 then (convert(dec(38,5),maxsize)*8)-(convert(dec(38,5),size)*8) when growth = 0 or maxsize = 0 or (maxsize = -1 and growth = 0) then 0  else (convert(dec(38,5),d.unused_size)/1024) end , 0)) from  #disk_drives d where  '
-- Protect againt using "collate" on SQL 7 compatible dbs
if (@cmptlevel > 70)
