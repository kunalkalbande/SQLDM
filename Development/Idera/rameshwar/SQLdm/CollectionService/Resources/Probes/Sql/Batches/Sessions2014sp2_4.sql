...
Set @SQL = (
	SELECT {4} N'Select ''' + Cast(session_id as nvarchar) + ''', '''', ''''; begin try IF(IS_SRVROLEMEMBER(''sysadmin'') = 1) BEGIN dbcc inputbuffer(' + Cast(session_id as nvarchar) + N') with no_infomsgs END ELSE BEGIN SELECT event_type, parameters, event_info FROM sys.dm_exec_input_buffer(' + Cast(session_id as nvarchar) + N', NULL) END end try begin catch select ''RPC Event'',0,''(unknown)'' end catch; '
	FROM #TempdbQueries
