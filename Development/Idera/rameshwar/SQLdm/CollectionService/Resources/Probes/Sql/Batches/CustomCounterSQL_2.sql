...
begin
	...
	begin
		if (not exists(select counter_name  from sys.dm_os_performance_counters
		...
		begin
			if (not exists(select instance_name from sys.dm_os_performance_counters
				where lower(object_name) = lower(@sysperfinfoname + ':{0}') and
...
ELSE
BEGIN
	SELECT isnull(convert(dec(38,0),NULL),0)
	select N'The counter name does not exist in sys.dm_os_performance_counters for the object.'
END