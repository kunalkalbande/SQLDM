...
--------------------------------------------------------------------------------
--  Batch: Server Basics - 2008 Sp1R2
--  Tables: None
...
--SQLdm 10.0 (Tarun Sapra): Minimal Cloud Support: In case of rds/azure, user is assummed to be 'sysadmin'
...
--START: SQLdm 10.0 (Tarun Sapra): Minimal Cloud Support, the following command is not supported if the sql server is hosted on azure instance
DECLARE @tempVar_ServerBasics_1 TABLE
(
	[Index] int, 
	[Name] nvarchar(1024),
...
INSERT INTO @tempVar_ServerBasics_1 VALUES(1,'ProductVersion',NULL,CONVERT(NVARCHAR(MAX),SERVERPROPERTY('ProductVersion'))) 
SELECT * FROM @tempVar_ServerBasics_1
...
--SQLdm 10.0 (Tarun Sapra)- NOT AVAILABLE IN RDS/AZURE
--RRG: This may work with Azure SQL DB Managed Instance since sp_configure is supported
--     Needs to be tested

--if (select count(*) from syscurconfigs where value <> 1 and config = 518) > 0 
--	begin 
...
begin
	...
	begin
		...
		--exec @service_status = xp_servicecontrol N'querystate', @agentservicename 
		--if @service_status = 1 
		...
		--------------------------------------------------------------------------------
		--START: SQLdm 10.0 (Tarun Sapra)- Mininmal Cloud Support: Permission Denied
		--exec @service_status = xp_servicecontrol N'QUERYSTATE',@fulltextservicename
	...
	begin
		...
		select @disableole = {1}
		--SQLdm 10.0 (Tarun Sapra)- In case of azure hosted sql server, this block wont be executed. Since ole is disabled by default
		if (@disableole = 0)
		begin

			select @curconfig = value from syscurconfigs where config = 16388 
