...
select
	p.object_id,
	DataSizeKB = sum(case when it.internal_type in (202,204) then 0 when p.index_id < 2 then convert(dec(12,0),au.data_pages) else 0 end) * 8,
	TextSizeKB = sum(case when it.internal_type in (202,204) then 0 when au.type != 1 then convert(dec(12,0),au.used_pages) else 0 end) * 8, 
...
select 
	...
	IndexSizeKB = 
		case 
			...
			- size.DataSizeKB - size.TextSizeKB) -- Subtract all data pages
			,0) end,
