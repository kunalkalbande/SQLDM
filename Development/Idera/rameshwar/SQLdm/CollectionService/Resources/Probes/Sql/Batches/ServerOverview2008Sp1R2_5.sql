...
--------------------------------------------------------------------------------\
		...
		ELSE
        BEGIN
          -- Operating System Version (Reference https://msdn.microsoft.com/en-us/library/windows/desktop/ms724832(v=vs.85).aspx)
          SELECT 'ProductName' AS Value, 
            CASE windows_release 
             WHEN '5.0' THEN 'Windows 2000'
             WHEN '5.1' THEN 'Windows XP'
             WHEN '5.2' THEN 'Windows Server 2003/Windows Server 2003 R2/Windows XP 64-Bit Edition'
		...
		begin 
			...
			begin
				...
				select @command = 'use [' + replace(@dbname,char(93),char(93)+char(93)) + '] '
								   ...
								   +'insert into #totaldbsizes select CONVERT(DECIMAL(15, 0), UsedExtents) * CONVERT(DECIMAL(15, 0), 64) from #filestats'
        ...
        ELSE
        BEGIN
	        SELECT CONVERT(NVARCHAR(MAX), NULL) AS [Database Name],
	    	       CONVERT(REAL, NULL) AS [Log Size (MB)],
	    		   CONVERT(REAL, NULL) AS [Log Space Used (%)],
	    		   CONVERT(INT, NULL) AS [Status];
        END
