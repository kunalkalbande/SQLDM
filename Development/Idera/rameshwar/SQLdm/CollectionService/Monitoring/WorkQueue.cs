...
namespace Idera.SQLdm.CollectionService.Monitoring
{
    ...
    public abstract class WorkQueue<T>
    {
        ...
        protected void CreateWorker()
        {
            ...
            thread.Name = "ScheduledRefresh:" + thread.ManagedThreadId;
        ...
        public void Enqueue(T item)
        {
            ...
            {
                queue.Enqueue(item);
                LOG.DebugFormat("WorkQueue - waitingWorkers: {0}, activeWorkers: {1}, queueCount: {2}", waitingWorkers.Count, activeWorkers.Count, queue.Count);
                while (strategy.AllowNewWorker(waitingWorkers.Count, activeWorkers.Count, queue.Count))
                {
                    CreateWorker();
                }                
