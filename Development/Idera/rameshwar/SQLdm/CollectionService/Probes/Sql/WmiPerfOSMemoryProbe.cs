...
namespace Idera.SQLdm.CollectionService.Probes.Sql
{
    ...
    /// </summary>
    class WmiPerfOSMemoryProbe: SqlBaseProbe
    {
        #region fields

        private WmiPerfOSMemorySnapshot snapshot = null;
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="WmiPerfOSMemoryProbe"/> class.
        /// </summary>
        ...
        public WmiPerfOSMemoryProbe(SqlConnectionInfo connectionInfo, WmiConfiguration configuration, int? cloudProviderId)
            : base(connectionInfo)
        {
            LOG = Logger.GetLogger("WmiPerfOSMemoryProbe");
            snapshot = new WmiPerfOSMemorySnapshot(connectionInfo.InstanceName);
        ...
        private void StartDirectCollector()
        {
            ...
            _wmicollector.Query = new WqlObjectQuery("select PagesPersec, Timestamp_Sys100NS from Win32_PerfRawData_PerfOS_Memory");
            ...
            else
            {
                ...
                if (result != null && result.Count > 0)
                {
                    foreach (ManagementBaseObject mbo in result)
                    {
                        try
                        {
                            var pagesPersec = WmiCollector.GetPropertyValueOrDefault<uint>(mbo, "PagesPersec");
                            var timestamp_Sys100NS = WmiCollector.GetPropertyValueOrDefault<ulong>(mbo, "Timestamp_Sys100NS");
                            snapshot.WmiPerfOSMemory.Rows.Add(pagesPersec, timestamp_Sys100NS);
                        }
