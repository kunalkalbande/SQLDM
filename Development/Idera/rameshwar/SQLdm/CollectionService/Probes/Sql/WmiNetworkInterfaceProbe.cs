...
namespace Idera.SQLdm.CollectionService.Probes.Sql
{
    ...
    /// </summary>
    class WmiNetworkInterfaceProbe: SqlBaseProbe
    {
        #region fields

        private WmiNetworkInterfaceSnapshot snapshot = null;
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="WmiNetworkInterfaceProbe"/> class.
        /// </summary>
        ...
        public WmiNetworkInterfaceProbe(SqlConnectionInfo connectionInfo, WmiConfiguration configuration, int? cloudProviderId)
            : base(connectionInfo)
        {
            LOG = Logger.GetLogger("WmiNetworkInterfaceProbe");
            snapshot = new WmiNetworkInterfaceSnapshot(connectionInfo.InstanceName);
        ...
        private void StartDirectCollector()
        {
            ...
            _wmicollector.Query = new WqlObjectQuery("select Name, CurrentBandwidth, Frequency_Sys100NS, Timestamp_Sys100NS, OutputQueueLength, PacketsPerSec, PacketsReceivedErrors, PacketsOutboundErrors, BytesTotalPerSec from Win32_PerfRawData_Tcpip_NetworkInterface");
                ...
                if (result != null && result.Count > 0)
                {
                    foreach (ManagementBaseObject mbo in result)
                    {
                        try
                        {
                            ...
                            var packetsPerSec = WmiCollector.GetPropertyValueOrDefault<uint>(mbo, "PacketsPerSec");
                            var outputQueueLength = WmiCollector.GetPropertyValueOrDefault<uint>(mbo, "OutputQueueLength");
                            var packetsReceivedErrors = WmiCollector.GetPropertyValueOrDefault<uint>(mbo, "PacketsReceivedErrors");
                            var packetsOutboundErrors = WmiCollector.GetPropertyValueOrDefault<uint>(mbo, "PacketsOutboundErrors");
                            var currentBandwidth = WmiCollector.GetPropertyValueOrDefault<ulong>(mbo, "CurrentBandwidth");
                            var bytesTotalPerSec = WmiCollector.GetPropertyValueOrDefault<ulong>(mbo, "BytesTotalPerSec");
                            var frequency_Sys100NS = WmiCollector.GetPropertyValueOrDefault<ulong>(mbo, "Frequency_Sys100NS");
                            var timestamp_Sys100N = WmiCollector.GetPropertyValueOrDefault<ulong>(mbo, "Timestamp_Sys100NS");
                            snapshot.WmiNetworkInterface.Rows.Add(name, packetsPerSec, outputQueueLength, packetsReceivedErrors, packetsOutboundErrors, currentBandwidth, bytesTotalPerSec, frequency_Sys100NS, timestamp_Sys100N);
                        }
