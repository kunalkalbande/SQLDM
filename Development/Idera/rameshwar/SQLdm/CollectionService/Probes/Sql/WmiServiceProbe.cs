...
namespace Idera.SQLdm.CollectionService.Probes.Sql
{
    ...
    /// </summary>
    class WmiServiceProbe: SqlBaseProbe
    {
        #region fields

        private WmiServiceSnapshot snapshot = null;
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="WmiServiceProbe"/> class.
        /// </summary>
        ...
        public WmiServiceProbe(SqlConnectionInfo connectionInfo, WmiConfiguration configuration, int? cloudProviderId)
            : base(connectionInfo)
        {
            this.cloudProviderId = cloudProviderId;
            LOG = Logger.GetLogger("WmiServiceProbe");
            snapshot = new WmiServiceSnapshot(connectionInfo.InstanceName);
        ...
        private void StartDirectCollector()
        {
            ...
            _wmicollector.Query = new WqlObjectQuery("select Name, ProcessId, State from Win32_Service");
                ...
                if (result != null && result.Count > 0)
                {
                    foreach (ManagementBaseObject mbo in result)
                    {
                        try
                        {
                            ...
                            var state = WmiCollector.GetReferencePropertyValue<string>(mbo, "State");
                            snapshot.WmiService.Rows.Add(name, processId, state);
                        }
