...
namespace Idera.SQLdm.CollectionService.Probes.Sql
{
    ...
    /// </summary>
    class WmiTCPProbe: SqlBaseProbe
    {
        #region fields

        private WmiTCPSnapshot snapshot = null;
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="WmiTCPProbe"/> class.
        /// </summary>
        ...
        public WmiTCPProbe(SqlConnectionInfo connectionInfo, WmiConfiguration configuration, int? cloudProviderId)
            : base(connectionInfo)
        {
            LOG = Logger.GetLogger("WmiTCPProbe");
            snapshot = new WmiTCPSnapshot(connectionInfo.InstanceName);
        ...
        {
            LOG.Info("TCPProbe's machinnamecallback");
        ...
        {
            LOG.Info("TCPProbe's starting direct collector");
            ...
            _wmicollector.Query = new WqlObjectQuery("select SegmentsPersec, SegmentsRetransmittedPerSec from Win32_PerfRawData_Tcpip_TCP");
        ...
        {
            LOG.Info("TCPProbe's starting direct wmi callback");
            LOG.VerboseFormat("wmiTCP collector ran in {0} milliseconds.", e.ElapsedMilliseconds);
            ...
            else
            {
                ...
                if (result != null && result.Count > 0)
                {
                    foreach (ManagementBaseObject mbo in result)
                    {
                        ...
                        {
                            LOG.Info("TCPProbe's starting direct wmi callback Error Occurred"); 
                            LOG.Error(ex);
                        }
                    }
                }
