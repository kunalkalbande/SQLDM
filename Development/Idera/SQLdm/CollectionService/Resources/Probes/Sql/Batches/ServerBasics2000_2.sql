...
ELSE
  SELECT 'ProductName' Value, 'UNKNOWN' Data
...
--------------------------------------------------------------------------------
if (select count(*) from syscurconfigs where value <> 1 and config = 518) > 0 AND IS_SRVROLEMEMBER('sysadmin') = 1
	begin 
...
begin
    ...
    IF IS_SRVROLEMEMBER('sysadmin') = 1 
    BEGIN
    	exec @service_status = xp_servicecontrol N'QUERYSTATE', N'MSSearch'
    END
    ELSE
    BEGIN
        SELECT @service_status = NULL
