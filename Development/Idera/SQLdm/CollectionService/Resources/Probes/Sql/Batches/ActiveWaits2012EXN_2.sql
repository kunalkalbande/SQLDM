--------------------------------------------------------------------------------
--  Batch: Active Waits Batch with extended events without extended events api for Azure
...
BEGIN
select @flagtime, (SELECT AVG(wait_time_ms)
			FROM sys.dm_os_wait_stats w
			WHERE wait_type IN 
				('DIRTY_PAGE_POLL','HADR_FILESTREAM_IOMGR_IOCOMPLETION','LAZYWRITER_SLEEP',
				'LOGMGR_QUEUE','REQUEST_FOR_DEADLOCK_SEARCH','XE_DISPATCHER_WAIT','XE_TIMER_EVENT')) as ms_ticks ;
END
ELSE
BEGIN
	SELECT @flagtime, NULL as ms_ticks
END
...
begin
	...
	ADD EVENT sqlos.wait_info(
		ACTION(mdmtargetpkg.mdmget_TimeStampUTC,'+
...
begin --we only get here if the trace exists
	...
	begin
		...
		select @sql = '	ALTER EVENT SESSION [' + @session +'] ON DATABASE
	...
	begin
		...
		exec ('alter event session [' + @session +'] ON DATABASE drop event user_event')
		exec ('alter event session [' + @session +'] ON DATABASE add event user_event')
	end
