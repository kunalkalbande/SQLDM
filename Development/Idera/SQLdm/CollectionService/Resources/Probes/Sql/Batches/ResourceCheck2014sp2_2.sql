...
begin
WITH numbered AS (select ROW_NUMBER() over (Partition by spid order by spid) as nr, * from sysprocesses p),
...
begin
	...
	Set @SQL = (
		SELECT  N'Select ''' + Cast(spid as nvarchar) + ''', '''', ''''; begin try dbcc inputbuffer(' + Cast(spid as nvarchar) + N') with no_infomsgs end try begin catch SELECT event_type,parameters,event_info FROM sys.dm_exec_input_buffer(' + Cast(spid as nvarchar) + N', NULL) end catch; '
