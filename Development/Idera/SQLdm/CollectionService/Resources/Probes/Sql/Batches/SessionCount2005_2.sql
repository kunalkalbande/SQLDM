...
--START (RRG): Works in both Azure SQL Database
--            Changed master..sysprocesses to sysprocesses
if (select isnull(object_id('tempdb..#processcpu'), 0)) = 0 
begin 
	create table #processcpu (spid smallint, cpu int);
	WITH numbered AS 
	(
		select ROW_NUMBER() over (Partition by spid order by spid) as nr
			,spid 
			,loginame 
			,hostname 
			,status 
...
--END (RRG): Works in both Azure SQL Database
