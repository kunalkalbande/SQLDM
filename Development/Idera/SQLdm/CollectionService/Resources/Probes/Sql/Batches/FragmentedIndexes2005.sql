...
declare @sysperfinfoname varchar(255);
declare @slashpos int;
declare @bufferPages bigint;

select @servername = cast(serverproperty('servername') as varchar(255));
select @servername = upper(@servername);
select @slashpos = charindex('\', @servername);

if @slashpos <> 0 
    begin select @sysperfinfoname = 'MSSQL$' + substring(@servername, @slashpos + 1, 30) end  
else 
    begin select @sysperfinfoname = 'SQLSERVER' end  
...
select 
	...
	[Schema]= (SELECT top 1 OBJECT_SCHEMA_NAME(object_id) FROM sys.objects o where o.object_id = object_id), 
