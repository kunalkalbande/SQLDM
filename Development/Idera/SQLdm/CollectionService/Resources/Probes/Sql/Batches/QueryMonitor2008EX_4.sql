--------------------------------------------------------------------------------
--  Batch: Query Monitor Batch ( Extended Events) 2008
...
declare @xemfilename nvarchar(4000) 
SET @xemfilename=ltrim(rtrim(@xefilename)) ;
Select  @xemfilename= @xemfilename +'.xem'
...
begin
	...
	select @sql2='ADD TARGET package0.asynchronous_file_target(SET filename= ''' + @xefilename + ''', metadatafile= '''+@xemfilename+''',max_file_size=({5}),max_rollover_files=({3}))
