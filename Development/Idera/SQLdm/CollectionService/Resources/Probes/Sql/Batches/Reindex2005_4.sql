...
begin
	...
	begin
		...
		BEGIN
		  if (len('{2}') > 0) and (@indexid is not null)
		  BEGIN
		    SELECT sum(avg_fragmentation_in_percent * page_count) / nullif(sum(page_count),0)
		    FROM 
		    master.sys.dm_db_index_physical_stats(DB_ID(), OBJECT_ID(replace(@qualified_table_name,char(39),char(39)+char(39))), @indexid, null, 'limited') d
		  ...
		  BEGIN
		    SELECT sum(avg_fragmentation_in_percent * page_count) / nullif(sum(page_count),0)
		    FROM 
		    master.sys.dm_db_index_physical_stats(DB_ID(), OBJECT_ID(replace(@qualified_table_name,char(39),char(39)+char(39))), 1, null, 'limited') d
		    WHERE  alloc_unit_type_desc = 'IN_ROW_DATA' 
		  END
		END
