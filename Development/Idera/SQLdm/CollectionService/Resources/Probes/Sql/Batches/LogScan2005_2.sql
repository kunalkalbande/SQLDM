...
insert into tempdb..#LogScanVars(StartDateTime,hostname) values (GETUTCDATE(), host_name())
...
select 
	...
	@agentlogtype = {1},
	@errorlogtype = {2},
	@skiperrorlog = 1, -- {3}, -- Skip agent error log for Azure
	@skipagentlog = 1, -- {4}, -- Skip agent log for Azure
	@noeventscheck = 0,
...
begin
	...
	begin
		...
		execute dbo.xp_readerrorlog 0, 1, NULL, NULL,  @beginDateTime, @now
...
Begin
	...
	insert into @logreader 
		execute dbo.xp_readerrorlog @archiveNo,1,null,null,@StartDateTime,@now,N'asc'; 
...
Begin
	if @@FETCH_STATUS <> -2
	insert into @logreader 
		execute dbo.xp_readerrorlog @archiveNo,2,null,null,@StartDateTime,@now,N'asc'; 
