--------------------------------------------------------------------------------
--  Batch: Failed Jobs 2005
--  Tables: msdb..sysjobhistory, msdb..sysjobsteps, msdb..sysjobs, 
--  msdb..syscategories,
--	Variables:	[0] - job filters where clause
--				[1] - s.on_fail_action = 2 (if job step alerting is turned off)
--				[2] - h.job_id not in (select [job id] from #tempfailedjobs) (if job step alerting is turned off)
--------------------------------------------------------------------------------\
declare @previnstance int, @treatfailureascritical int, @flag int 
...
declare @parmdef nvarchar(1000), @command nvarchar(max)

set @parmdef = '@previnstance int, @treatfailureascritical int, @flag int'
...
select 
	...
	''job id'' = j.job_id,
	''Job Name'' = isnull(j.name,''(Unknown)''),
	''step name'' = isnull(h.step_name,''(Unknown)''),
	''step id'' = h.step_id, 
	''id'' = h.sql_message_id, 
...
exec sp_executesql @command, @parmdef, @previnstance = @previnstance, @treatfailureascritical = @treatfailureascritical, @flag = @flag
