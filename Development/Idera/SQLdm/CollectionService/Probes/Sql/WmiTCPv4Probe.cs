...
namespace Idera.SQLdm.CollectionService.Probes.Sql
{
    ...
    /// </summary>
    class WmiTCPv4Probe: SqlBaseProbe
    {
        #region fields

        private WmiTCPv4Snapshot snapshot = null;
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="WmiTCPv4Probe"/> class.
        /// </summary>
        ...
        public WmiTCPv4Probe(SqlConnectionInfo connectionInfo, WmiConfiguration configuration, int? cloudProviderId)
            : base(connectionInfo)
        {
            LOG = Logger.GetLogger("WmiTCPv4Probe");
            snapshot = new WmiTCPv4Snapshot(connectionInfo.InstanceName);
        ...
        {
            LOG.Info("TCPv4Probe's machinnamecallback");
        ...
        {
            LOG.Info("TCPv4Probe's starting direct collector");
            ...
            _wmicollector.Query = new WqlObjectQuery("select SegmentsPersec, SegmentsRetransmittedPerSec from Win32_PerfRawData_Tcpip_TCPv4");
        ...
        {
            LOG.Info("TCPv4Probe's starting direct wmi callback");
            LOG.VerboseFormat("wmiTCPv4 collector ran in {0} milliseconds.", e.ElapsedMilliseconds);
            ...
            else
            {
                ...
                if (result != null && result.Count > 0)
                {
                    foreach (ManagementBaseObject mbo in result)
                    {
                        ...
                        catch (Exception ex) { LOG.Info("TCPv4Probe's starting direct wmi callback Error Occurred"); 
                            LOG.Error(ex); }
                    }
                }
            }
