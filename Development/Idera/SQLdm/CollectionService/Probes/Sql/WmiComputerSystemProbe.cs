...
namespace Idera.SQLdm.CollectionService.Probes.Sql
{
    ...
    /// </summary>
    class WmiComputerSystemProbe: SqlBaseProbe
    {
        #region fields

        private WmiComputerSystemSnapshot snapshot = null;
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="WmiComputerSystemProbe"/> class.
        /// </summary>
        ...
        public WmiComputerSystemProbe(SqlConnectionInfo connectionInfo, WmiConfiguration configuration, int? cloudProviderId)
            : base(connectionInfo)
        {
            LOG = Logger.GetLogger("WmiComputerSystemProbe");
            snapshot = new WmiComputerSystemSnapshot(connectionInfo.InstanceName);
        ...
        private void StartDirectCollector()
        {
            ...
            _wmicollector.Query = new WqlObjectQuery("select DomainRole from Win32_ComputerSystem where Name='" + machineName + "'");
            ...
            else
            {
                ...
                if (result != null && result.Count > 0)
                {
                    foreach (ManagementBaseObject mbo in result)
                    {
                        try
                        {
                            var domainRole = WmiCollector.GetPropertyValueOrDefault<uint>(mbo, "DomainRole");
                            snapshot.WmiComputerSystem.Rows.Add(domainRole);
                        }
