...
namespace Idera.SQLdm.CollectionService.Probes.Sql
{
    ...
    /// </summary>
    class WmiNetworkRedirectorProbe: SqlBaseProbe
    {
        #region fields

        private WmiNetworkRedirectorSnapshot snapshot = null;
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="WmiNetworkRedirectorProbe"/> class.
        /// </summary>
        ...
        public WmiNetworkRedirectorProbe(SqlConnectionInfo connectionInfo, WmiConfiguration configuration, int? cloudProviderId)
            : base(connectionInfo)
        {
            LOG = Logger.GetLogger("WmiNetworkRedirectorProbe");
            snapshot = new WmiNetworkRedirectorSnapshot(connectionInfo.InstanceName);
        ...
        private void StartDirectCollector()
        {
            ...
            _wmicollector.Query = new WqlObjectQuery("select NetworkErrorsPerSec from Win32_PerfRawData_PerfNet_Redirector");
            ...
            else
            {
                ...
                if (result != null && result.Count > 0)
                {
                    foreach (ManagementBaseObject mbo in result)
                    {
                        try
                        {
                            var networkErrorsPerSec = WmiCollector.GetPropertyValueOrDefault<uint>(mbo, "NetworkErrorsPerSec");
                            snapshot.WmiNetworkRedirector.Rows.Add(networkErrorsPerSec);
                        }
