--------------------------------------------------------------------------------
--  Batch: Replication Distribution Details 2000
...
'Publisher' = pub.srvname + '.' + s.publisher_db, 
'Distributor' = (select srvname from master..sysservers where srvid = 0),
'Subscriber' = sub.srvname + '.' + s.subscriber_db,
...
'SampleTime' = GetUTCDate()  into #tempLatency
 ...
 left join master..sysservers pub on s.publisher_id = pub.srvid
 left join master..sysservers sub on s.subscriber_id = sub.srvid
...
left outer join (select maxSeqPerAgent.agent_id, t.publisher_database_id, s.subscriber_id,  max(t.entry_time) as entry_time
...
group by t.publisher_database_id, s.subscriber_id, maxSeqPerAgent.agent_id) as dblatency on dblatency.publisher_database_id = s.publisher_database_id 
 and dblatency.subscriber_id = s.subscriber_id
 and dblatency.agent_id = s.agent_id
 where s.status >= 0
 and ((s.subscriber_id >= 0 and exists(select agent_id from MSsubscriptions s1 where s1.publisher_db = s.publisher_db and s1.subscriber_id >= 0) and s.subscriber_db = N'{3}' and sub.srvname = N'{4}')
 ...
 and lower(pub.srvname) = lower(N'{1}') and publ.publication = N'{2}'
group by s.publisher_database_id,s.publisher_id,s.publisher_db,s.publication_id,publ.publication,s.subscriber_id,s.subscriber_db,s.agent_id,pub.srvname,sub.srvname,
