...
select top 1
	...
	commandtext = {1},
	runtime = datediff(s,transaction_begin_time,getdate()),
...
where
    exists (select * from sys.dm_tran_database_transactions dtdt 
                where dtdt.transaction_id = t.transaction_id and dtdt.database_transaction_begin_lsn is not NULL)
	{0}
order by 
	datediff(s,transaction_begin_time,getdate()) desc
