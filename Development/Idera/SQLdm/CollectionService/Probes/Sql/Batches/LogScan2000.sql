...
--  Batch: Log Scan 2000
--	Variables:	
--	[0] - Start date for scan - local to monitored server
--	[1] - Rowcount max to return (flood control)
--	[2] - Enumeration to return for Agent Logs
--	[3] - Enumeration to return for SQL Logs
--	[4] - Skip error log (1 if true)
--	[5] - Skip agent log (1 if true)
--------------------------------------------------------------------------------
set nocount on
use master
set rowcount 0
	...
	begin
		...
		begin
			...
			if 
				...
				from #fileDetails)
				>= dateadd(mi,-62,@StartDateTime)  -- Due to daylight savings timestamps may be incorrect up to 1 hour
			begin
