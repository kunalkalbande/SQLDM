	...
	begin 
		...
		begin 
			select @command = 'select db_name(@DBID),o.name,''dataspace'' = sum(case when indid in (0,1) then (convert(dec(12,0),dpages) * 8)  else 0 end), '
				...
				+ '''textused'' = sum(case when indid = 255 then (convert(dec(12,0),used) * 8) else 0 end), sum(case when indid < 2 then i.rows else 0 end), case when o.type = ''S'' then 1 else 0 end, ISNULL(u.name,''unknown''), ' 
