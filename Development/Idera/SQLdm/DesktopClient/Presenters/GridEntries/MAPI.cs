...
namespace Idera.SQLdm.DesktopClient.Presenters.GridEntries
{
    internal class MAPI
    {
        ...
        public int SendMailPopup(Form f, string strSubject, string strBody)
        {
            return SendMail(f, strSubject, strBody, MAPI_LOGON_UI | MAPI_DIALOG);
        }

        public int SendMailDirect(Form f, string strSubject, string strBody)
        {
            return SendMail(f, strSubject, strBody, MAPI_LOGON_UI);
        }

        [DllImport("MAPI32.DLL")]
        static extern int MAPISendMail(IntPtr sess, IntPtr hwnd, MapiMessage message, int flg, int rsv);

        int SendMail(Form f, string strSubject, string strBody, int how)
        {
            ...
            try
            {
                ...
                m_lastError = MAPISendMail(new IntPtr(0), (null == f) ? new IntPtr(0) : f.Handle, msg, how, 0);
                if (m_lastError > 1) throw new ApplicationException("Failed to send mail! " + GetLastError());
            }
            finally
            {
                Cleanup(ref msg);
