...
{
   public partial class MetricThresholds : Idera.SQLdm.DesktopClient.Views.Reports.ReportControls.ReportContol
   {
      private static readonly Logger Log = Logger.GetLogger("Metric Thresholds");
      private List<ReportParameter> passthroughParameters = new List<ReportParameter>(1);

      public MetricThresholds()
      {
          InitializeComponent();
          AdaptFontSize();
      }
      ...
      public override void InitReport()
      {
         ...
         ReportType = ReportTypes.MetricThresholds;
         RunReport(true);
      }
      ...
      override protected void bgWorker_DoWork(object sender, DoWorkEventArgs e)
      {
         ...
         using (Log.DebugCall())
         {
            ...
            ReportDataSource dataSource = new ReportDataSource("MetricThresholds");
            dataSource.Value = RepositoryHelper.GetReportData("p_GetMetricThresholdDetails", localReportData.instanceID, null);
               ...
               else
               {
                  try
                  {
                     ...
                     using (Stream stream = GetType().Assembly.GetManifestResourceStream(
                         "Idera.SQLdm.DesktopClient.Views.Reports.ReportDefinitions.MetricThresholds.rdl"))
                     {
                     ...
                     reportViewer.LocalReport.DisplayName = "MetricThresholds";
