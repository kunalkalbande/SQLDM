...
{
	//SQLdm 9.1 (Ankit Srivastava) --Disk Size Reports --Added new report view for Disk Space Usage Report
    public partial class DiskSpaceUsage : ReportContol
    {
        private static readonly Logger Log = Logger.GetLogger("Disk Space Usage");
        ...
        private List<string> curDisks = new List<string>();
        ...
        protected override void InitInstanceCombo()
        {
            ...
            instanceSelectOne = new ValueListItem(null, "< All Servers >");
        ...
        //{
         //   base.ResetFilterCriteria();
          //  cboOrderBy.SelectedItem = cboOrderBy.Items[0];
        ...
        public override void InitReport()
        {
            ...
            ReportType = ReportTypes.DiskSpaceUsage;
            ...
            if (drillthroughParams.ContainsKey("SortParameter"))
            {
                try
                {
                    cboOrderBy.SelectedItem = Convert.ToString(drillthroughParams["SortParameter"][0]);
                }
                catch
                {
                    cboOrderBy.SelectedItem = cboOrderBy.Items[0];
        ...
        public override Dictionary<string, object> GetReportParmeters()
        {
            ...
            SetReportParameters();
            //SQLDM-26805 - Vamshi Krishna - Code changes to exclude the disks in the disk space usage report
            if (reportParameters.ContainsKey("DiskNameFilter")) reportParameters.Remove("Disk");
            if (reportParameters.ContainsKey("SortType")) reportParameters.Remove("SortType");//not required for deployement
            if (reportParameters.ContainsKey("SortOrder")) reportParameters.Remove("SortOrder");//not required for deployement
        ...
        protected override void SetReportParameters()
        {
            ...
            if(cboOrderBy.SelectedItem != null)
            {

                //this is for deployment
                reportParameters.Add("SortParameter", cboOrderBy.SelectedItem.ToString());

                string sortType="InstanceName";
                string sortOrder="ASC";
                switch(cboOrderBy.SelectedIndex)
                {
                    case 0:
                        sortType="InstanceName";
                        sortOrder="ASC";
                        break;
                    case 1:
                        sortType="FreeDiskSpaceMB";
                        sortOrder="ASC";
                        break;
                    case 2:
                        sortType="FreeDiskSpaceMB";
                ...
                reportParameters.Add("SortType", sortType);
                reportParameters.Add("SortOrder", sortOrder);
            }
            ...
            if (Disks != null && Disks.Count > 0)
            {
                ...
                reportParameters.Add("DiskNameFilter", Disks);
            }
            else
            ...
            using (Log.DebugCall())
            {
                ...
                if (Disks != null && Disks.Count > 0)
                {
                    ...
                    for (var i = 0; i < Disks.Count; i++)
                        if(i==0) diskDetails = Disks[i];
                    else diskDetails = diskDetails + ","+ Disks[i];

                    passthroughParameters.Add(new ReportParameter("DiskNameFilter", diskDetails));
                }
                ...
                passthroughParameters.Add(new ReportParameter("SortParameter", (string)localReportData.reportParameters["SortParameter"]));


                Log.Debug("localReportData.reportType = ", localReportData.reportType);
                localReportData.dataSources = new ReportDataSource[1];

                dataSource = new ReportDataSource("DiskSpaceUsage");
                ...
                //SQLDM-26805 - Vamshi Krishna - Code changes to exclude the disks in the disk space usage report
                dataSource.Value = RepositoryHelper.GetReportData("p_GetDiskSpaceUsageReportData",
                    GetServerIdXml(servers), diskDetails, (string)localReportData.reportParameters["SortType"], (string)localReportData.reportParameters["SortOrder"]);
                    ...
                    else
                    {
                        try
                        {
                            ...
                            using (Stream stream = GetType().Assembly.GetManifestResourceStream(
                                "Idera.SQLdm.DesktopClient.Views.Reports.ReportDefinitions.DiskSpaceUsage.rdl"))
                            {
                            ...
                            reportViewer.LocalReport.DisplayName = "DiskSpaceUsage";
        ...
        protected void diskBrowseButton_Click(object sender, EventArgs e)
        {
            ...
            {
                //Store the previously selected disks before clearing it.
                List<string> CheckedDisks = new List<string>();
                CheckedDisks = Disks;
                curDisks = null;
                DiskExcludeDialog dlg =
                    new DiskExcludeDialog(Settings.Default.ActiveRepositoryConnection.ConnectionInfo,
                                              ...
                                              "Check one or more disks to exclude from the report.", "Select Disks");
                ...
                else
                {
                    ...
                    {
                        dlg.SelectedDisk = CheckedDisks[0];
                    }
                }
                ...
                {
                    string diskDetails = "";
                    if (CheckedDisks.Count > 0)
                    {
                        for (var i = 0; i < CheckedDisks.Count; i++)
                            if (i == 0) diskDetails = CheckedDisks[i];
                            else diskDetails = diskDetails + "," + CheckedDisks[i];
                    }
                   
                    
                    diskTextbox.Text = diskDetails;
                }
                catch
                {
                    Disks = null;
