...
namespace Idera.SQLdm.Utility.Build
{
    /// <summary>
            ...
            switch (RevisionType) {
                ...
                case RevisionNumberAlgorithm.Increment:
                    if (newBuildNumber != version.Build) {
                        // reset revision number to 0 if the build number
                        // has changed
                        newRevsionNumber = 0;
                    } else {
                        // increment the revision number if this is a revision
                        // of the same build
                        newRevsionNumber = version.Revision == -1 || version.Revision == 0 ? 1 : version.Revision + 1;
                    }
                    break;
