...
namespace Idera.SQLdm.Common.Notification.Providers
{
    ...
    using Events;
    [Serializable]
    public class SCOMEventDestination : NotificationDestinationInfo
    {
        
        public SCOMEventDestination()
        {
            // get the id of the notification provider from attributes
            GuidAttribute attribute =
                Attribute.GetCustomAttribute(typeof(SCOMEventNotificationProviderInfo), typeof(GuidAttribute)) as GuidAttribute;
            if (attribute != null)
                this.ProviderID = new Guid(attribute.Value);
        }

        public SCOMEventDestination(NotificationDestinationInfo destination)
        {
