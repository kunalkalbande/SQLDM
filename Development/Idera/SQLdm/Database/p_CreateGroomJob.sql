...
begin
	...
	begin -- update the jobstep
		...
		EXEC msdb.dbo.sp_update_jobstep 
			@job_id=@jobId, 
			@step_name=N'AG Primary', 
			@step_id=1, 
			@cmdexec_success_code=0, 
			@on_success_action=3, 
			@on_fail_action=1, -- Exit preventing to continue with the rest of the steps
			@retry_attempts=0, 
			@retry_interval=0, 
			@os_run_priority=0, @subsystem=N'TSQL', 
			@command=N'exec Grooming.p_VerifyAGNode', 
	...
	-- create or update the jobstep
	IF (@JobExists=1 AND EXISTS(SELECT step_name FROM msdb.dbo.sysjobsteps WHERE job_id=@jobId AND step_id = 4))
	begin -- update the jobstep
