//------------------------------------------------------------------------------
// <copyright file="RegistrationServiceConfiguration.cs" company="Idera, Inc.">
...
namespace Idera.SQLdm.RegistrationService.Configuration
{
    ...
    /// </summary>
    public sealed class RegistrationServiceConfiguration
    {
        private static BBS.TracerX.Logger LOG = BBS.TracerX.Logger.GetLogger("RegistrationServiceConfiguration");
        ...
        private static RegistrationServiceConfig publishedConfiguration;

        private static bool enableEvents;
        private static bool configDirty;
        private static bool needRefresh;

        static RegistrationServiceConfiguration()
        {
            syncRoot = new ReaderWriterLock();
            needRefresh = true;

            connectInfo = new SqlConnectionInfo();
            connectInfo.ApplicationName = "SQL Diagnostic Manager Registration Service" ;
        ...
        public static void LogConfiguration()
        {
            ...
            buffer.AppendFormat("Registration Service Instance Name: {0}\n", InstanceName);
            buffer.AppendFormat("Registration Service Remoting Port: {0}\n", ServicePort);
            buffer.AppendFormat("Registration Service Data Path: {0}\n", DataPath);
        ...
        private static void internal_refresh()
        {
            ...
            try
            {
                RegistrationServiceElement element = GetRegistrationServiceElement();
        ...
        {
            RegistrationServiceElement oldElement = GetRegistrationServiceElement();
            ...
            try
            {
                if (configDirty)
                {
                    RegistrationServiceElement element = new RegistrationServiceElement();
...
//                    element.HeartbeatIntervalSeconds = (int)heartbeatInterval.TotalSeconds;
            ...
            get
            {
                ...
                {   // check to make sure someone else didn't update it already
                    if (String.IsNullOrEmpty(resolvedDataDirectory))
                    {
                        ...
                        if (!path.EndsWith("" + Path.DirectorySeparatorChar + "RegistrationService", StringComparison.InvariantCultureIgnoreCase))
                        {
                            if (!path.EndsWith("" + Path.DirectorySeparatorChar + "Data", StringComparison.InvariantCultureIgnoreCase))
                                path = Path.Combine(path, "Data");

                            path = Path.Combine(path, "RegistrationService");
                        }
        ...
        public static RegistrationServiceElement GetRegistrationServiceElement()
        {
        ...
        internal static void PublishConfiguration()
        {
            try
            {
                ...
                Assembly assembly = typeof(RegistrationServiceConfiguration).Assembly;
                ...
                RegistrationServiceConfig wmiConfig = new RegistrationServiceConfig();
...
//                wmiConfig.Published = true;
    ...
    [ManagedName("RegistrationServiceConfiguration")]
    public class RegistrationServiceConfig  // : System.Management.Instrumentation.Instance
    {
